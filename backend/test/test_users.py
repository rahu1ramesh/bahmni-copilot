...
def test_create_user(test_db: Session, auth_headers):
    user_data = {"name": "New User", "user_name": "newuser", "email": "newuser@example.com", "password": "newpassword"}
...
def test_create_user_with_existing_user_name(test_db: Session, auth_headers):
    user_data = {"name": "New User", "user_name": "admin", "email": "newuser@example.com", "password": "newpassword"}
    response = client.post("/api/users/", json=user_data, headers=auth_headers)
    assert response.status_code == 409


def test_create_user_with_existing_email(test_db: Session, auth_headers):
    user_data = {"name": "New User", "user_name": "newuser", "email": "admin@example.com", "password": "newpassword"}
    response = client.post("/api/users/", json=user_data, headers=auth_headers)
    assert response.status_code == 409
...
def test_get_user_by_id(test_db: Session, auth_headers):
    user_data = UserCreate(
        name="Test User", user_name="testuser", email="testuser@example.com", password="testpassword"
    )
